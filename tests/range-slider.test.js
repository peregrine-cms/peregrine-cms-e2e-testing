/**
 * single-test run:
 * npm run test -- ./tests/range-slider.test.js
 * npm run test:headful -- ./tests/range-slider.test.js
 */
...
async function expectEmpty() {
  expect(await topPadding.grabInputValue()).toBe('');
  expect(await topPadding.grabRangeValue()).toBe('50');
}

async function expectValue(val) {
  expect(await topPadding.grabInputValue()).toBe(val.toString());
  expect(await topPadding.grabRangeValue()).toBe(val.toString());
}
